...
| 生成时段 | 尝试耗时减去首字延迟 | 现算，不落库 |
| 请求总耗时 | 从收到客户端请求到写完响应的耗时 | `request_logs.totalDurationMilliseconds` |

**首字延迟是尝试级事实。** 「首字」指上游产出的第一个**真正内容**：只说明「连接还活着」或「我结束了」的帧——纯角色帧、纯用量帧、`[DONE]`——都不算。整包响应（非流式）没有「首字」这个时刻，为 `null`；它不是 0，是「这件事不存在」。它随尝试结果一并落库，与正文采集开关无关。

**请求总耗时是另一条轴。** 它是整条链路的长度，包含上游重试与故障转移，**不等于任何一次尝试的耗时**。因此它永远不能当速度的分母，也不参与「首字多快」的表述。
...
$$\text{TPS} = \frac{\text{输出 Token}}{\text{生成时段}}$$

分母是**生成时段**，而不是尝试耗时，更不是请求总耗时：

- 首字延迟回答「多久开始出字」，速度回答「出字多快」，两者是可分开的体验维度。把等待时间算进分母，会让两个首字延迟差十倍、出字速度相同的模型显示成不同的速度。
- 首字延迟缺失（整包响应）时按 0 处理，而不是判为「算不出来」：只有这样，流式与整包响应用的才是同一个公式，两种传输形态的模型才可比。
- 生成时段下限钳在 0：首字延迟记到整段耗时之外只会是采集误差，不该变成负的分母。

**分子与分母必须来自同一批样本。** 哪个样本的输出 Token 进了分子，它的生成时段就必须进分母；反过来，算不出生成时段的样本，其 Token 也不能只留在分子里——那会让比值被单方面抬高。因此：

- 只取**成功的尝试**参与统计：失败的尝试没有完整输出，其 Token 数不具备「单位时间产出」的含义。
- 输出 Token 为空（上游没报用量）、为 0，或生成时段不为正的样本**整体剔除**，分子分母一起剔。
- **分母趋近 0 不是极高速度，是没有速度。** 这类样本为 `null`（界面显示 `—`），而不是一个天文数字。这是本项目最典型的口径错误：一次首字延迟吃掉整段耗时的尝试，会除出几千 TPS 的假值。
- 没有样本够格时为空，不是 0——「没有速度」和「速度为零」是两件事。

### 平均速度

多请求、多尝试的平均速度用**先求和再相除**：$\sum \text{输出 Token} \div \sum \text{生成时段}$。
...
- **TTFT 分布按首字延迟分桶，不按总耗时分桶。** 一张「按 p95 均分」的分布图若按总耗时切区间，就答不了「首字多快」这个问题。
...
| 输出速度 | `request_usages` / `request_attempts` | 输出 Token 除以服务该请求那次尝试的**生成时段**（尝试耗时减首字延迟），两者同一批样本现算，不落库。口径见[延迟与速度指标口径](#延迟与速度指标口径) |
