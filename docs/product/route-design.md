...
- 页头右上角的动作各模式自便，但**次序在两边是同一套**（编辑 → 验证 → 发布 → 回滚）：工作流模式是策略预设、试运行、保存、历史版本；规则模式是新增规则、试运行、保存、历史版本。同一个位置上的两套动作长得一样，换个模式不用重新找按钮。规则表没有策略预设（它没有可套用的图），图模式没有「新增」（画的每一笔都在画布上，没有可批量创建的既有对象）；
