...
| 平均输出 | 输出总量 ÷ 请求总数；小数位与输出速度同一档：10 及以上取整（`80`），10 以下保留一位小数（`8.3`） |
...
- **顶部卡片不放请求级平均延迟。** `request_logs.totalDurationMilliseconds` 把「一次 30 秒的长回答」和「一次 200 毫秒的短问答」平成一个数，它随响应体量漂移，既不对应任何一次请求，也无法用来判断链路快慢。问「快不快」有两个更窄也更准的口径：尝试级平均延迟（只算成功尝试）答「上游多快」，TTFT 分布答「多久开始出字」。同一张卡片换成**单次请求平均输出**（输出总量 ÷ 请求总数）：它同样随体量变化，但那正是它要描述的东西——这段时间里一次请求平均产出多少内容。
...
- 单次请求平均输出：输出 Token 总量 ÷ 请求总数。输出总量与请求总数都在请求级 `request_usages` / `request_logs` 上按同一时间窗取，且写在同一次扫描里，保证分子分母来自同一批样本；分子是**所有请求的输出之和**，不区分成功与否。
