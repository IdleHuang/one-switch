...
用量趋势与 TTFT 直方图的粒度**由查询范围推导**，不写死：`today` / `7d` / `30d` 各自算出一份「窗口 + 两张图的粒度」，规则集中在 `packages/contracts/source/analytics-buckets.ts`。服务端 SQL 分桶、标签生成、界面坐标轴取的是同一份结论——任何一处凭印象写死一个步长，就会出现「窗口 30 天、柱子还是 15 分钟」，或者「统计出来的桶」与「画出来的桶」对不上。

**趋势图的桶宽取「柱子数不超过 60 根」的最细一档。** 候选步长是 5 / 10 / 15 / 30 / 60 / 120 / 180 / 240 / 360 / 480 / 720 / 1440 分钟，全部整除 1440，所以桶边界落在本地整点或整天上——「每 3 小时」的柱子起点永远是 00:00 / 03:00 / 06:00，而不是从查询时刻往前推出来的随机分钟数。上限定在 60 而不是 24：卡片内宽约 800px，60 根柱子每根还能分到 13px，再细就只剩一条线；反过来「一周只画七个点」也读不出日内起伏，看不出哪天上午打了什么高峰。结果：今天从 5 分钟随当天时间一路粗到 30 分钟（最多 60 根），近 7 天 3 小时（57 根），近 30 天一天一根（31 根——两端各有一个不完整的桶，所以比「跨度 ÷ 步长」多一个）。
...
- **桶宽随响应一并回传**（`trendIntervalMs`）：界面据此决定刻度写 `06:00` 还是 `9/9`、卡片副标题写「每 3 小时用量」还是「每日用量」，不自己反推粒度。标签与桶宽都是**跨进程的显示数据**，对不上时（例如界面已更新、服务端还在发旧版的 `HH:MM` 标签且不带桶宽）两种降级都要落到「少说一句」而不是报错：认不出的标签照原样写出来，桶宽缺失就不写这句粒度说明。格式化函数里让 `Intl.DateTimeFormat.format(NaN)` 抛 `RangeError: Invalid time value`，代价是一整张图渲染不出来。
