...
- **顶部卡片不放请求级平均延迟。** `request_logs.totalDurationMilliseconds` 把「一次 30 秒的长回答」和「一次 200 毫秒的短问答」平成一个数，它随响应体量漂移，既不对应任何一次请求，也无法用来判断链路快慢。问「快不快」有两个更窄也更准的口径：尝试级平均延迟（只算成功尝试）答「上游多快」，TTFT 分布答「多久开始出字」。同一张卡片换成**缓存命中率**（缓存读取 Token ÷ 输入 Token 总量）：它回答的是另一个问题——这段时间里提示词缓存有没有被用上、成本结构有没有变。分子本就是分母的一部分（输入总量含缓存读取），两者同一口径相除才是命中率。

- **平均输出只在模型使用排行里出现，且分母是成功调用数。** 排行一行代表一个上游模型的调用情况，它的输出量由尝试级用量按 `status = 'success'` 筛出，所以分母必须是**成功调用数**；拿它去除以全部尝试数等于把失败尝试也算进分母，比值会被单方面压低。同理，没有成功调用的模型这一格为空而不是 0。
...
- 缓存命中率：缓存读取 Token ÷ 输入 Token 总量。两个总量都在 `request_usages` 上按同一时间窗取、写在同一轮扫描里；输入总量本就含缓存读取，所以这里是「分母的一部分 ÷ 分母」。窗口内没有任何输入时为空——没测到不是命中率为零。
- 模型使用排行里的平均输出：该模型成功调用的输出总量 ÷ 成功调用数。分子是尝试级用量按 `status = 'success'` 筛出的那部分，分母就必须是成功调用数；没有成功调用时为空。
